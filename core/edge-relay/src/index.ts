...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      const servedFromStale = sanitized.headers.get('X-CredLink-Relay-Stale') === '1';
      if (servedFromStale) {
        sanitized.headers.delete('X-CredLink-Relay-Stale');
      }
...
async function fetchUpstream(relayContext: RelayContext, request: Request): Promise<Response> {
  ...
  upstreamHeaders.set('User-Agent', 'credlink-edge-relay/1.0 (+https://credlink)');
  upstreamHeaders.set('X-CredLink-Relay-Tenant', sanitizeTenantId(relayContext.tenantId));

  const tsHeader = Math.floor(relayContext.timestamp).toString();
  upstreamHeaders.set('X-CredLink-Relay-Ts', tsHeader);

  if (relayContext.tenant.originSigningSecret) {
    const originSignature = await signOriginRequest(relayContext, tsHeader);
    upstreamHeaders.set('X-CredLink-Sign', originSignature);
  }
  ...
  try {
    ...
    if (stale) {
      ...
      fallback.headers.set('X-CredLink-Relay-Stale', '1');
      return fallback;
    }
  ...
  if (isServerError(response.status)) {
    ...
    if (stale) {
      ...
      staleResponse.headers.set('X-CredLink-Relay-Stale', '1');
