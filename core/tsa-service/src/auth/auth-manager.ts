...
export class AuthenticationManager {
  private tenants: Map<string, AuthenticatedTenant> = new Map();
  private apiKeys: Map<string, string> = new Map(); // apiKey -> tenantId
   ...
   */
  async authenticateRequest(apiKey: string): Promise<AuthenticationResult> {
   ...
   */
  async checkRateLimit(tenantId: string, window: 'minute' | 'hour' | 'day' = 'minute'): Promise<boolean> {
    // Input validation
   ...
   */
  async getTenantPolicy(tenantId: string, apiKey: string): Promise<TenantTSAPolicy | null> {
    // Input validation
  ...
  private initializeTenants(): void {
    ...
    const tenants: AuthenticatedTenant[] = [
      {
        tenantId: 'acme-corp',
   ...
   */
  private isTenantActive(tenant: AuthenticatedTenant): boolean {
    // Input validation
   ...
   */
  private getClientKey(apiKey: string): string {
   ...
   */
  private isRateLimitedForFailures(clientKey: string): boolean {
   ...
   * Validate tenant configuration integrity
   */
  private validateTenantIntegrity(tenant: AuthenticatedTenant): boolean {
