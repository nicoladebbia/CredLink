...
import { VerifyEventSystem, VerifyEvent, type WebhookConfig } from '../core/verify-events.js';
...
export class LicenseEnforcementAPI {
  private partnerConfigs = new Map<string, PartnerConfig>();
  private eventHistory: VerifyEvent[] = [];
  ...
  async verifyAsset(request: VerifyRequest): Promise<VerifyResponse> {
    ...
    try {
      ...
      let partnerConfig: PartnerConfig | undefined;
      
      // Find partner config that allows this origin
      this.partnerConfigs.forEach((config: PartnerConfig) => {
      ...
      let badgeState: 'ok' | 'warn' | 'block' = 'ok';
      let previewDegrade: { scale: number; blur_px: number } | undefined;
      ...
      const action: VerifyResponse['action'] = {
        show_badge: true,
      ...
      const response: VerifyResponse = {
        ...
        signals: {
          assertions: manifest.assertions?.map((a: any) => a.label) || [],
  ...
  async exportEvents(partnerId: string, filters: {
    ...
    let events = this.eventHistory.filter(event => {
      ...
      const hasWebhook = partnerConfig.webhooks.some(webhook => 
        VerifyEventSystem.shouldDeliverEvent(webhook, event)
      );
      
      if (!hasWebhook) return false;

      // Filter by date range
      if (filters.from && new Date(event.created) < new Date(filters.from)) return false;
      if (filters.to && new Date(event.created) > new Date(filters.to)) return false;
...
// ... (rest of the code remains the same)
  private async extractManifestHash(assetUrl: string): Promise<string> {
   ...
   * Fetch and verify manifest using CAI Verify
   */
  private async fetchAndVerifyManifest(_assetUrl: string, manifestHash: string): Promise<any> {
    // In a real implementation, this would use the CAI Verify SDK
    // For now, return a mock manifest with proper structure
    return {
      manifest_hash: manifestHash,
      claim_generator: 'CredLink v1.1',
   ...
   */
  private async emitVerificationEvent(request: VerifyRequest, response: VerifyResponse, _manifest: any): Promise<void> {
   ...
   */
  private async deliverEventsToPartners(event: VerifyEvent): Promise<void> {
      ...
      for (const webhook of config.webhooks) {
        if (VerifyEventSystem.shouldDeliverEvent(webhook, event)) {
          deliveryPromises.push(
            VerifyEventSystem.deliverWebhook(webhook, event).catch(error => {
   ...
   */
  async configurePartner(partnerId: string, config: Partial<PartnerConfig>): Promise<void> {
