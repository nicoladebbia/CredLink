...
export class JUMBFBuilder {
  ...
  async build(options: {
    ...
    // 4. Create Signature Box with real signature if provided
    const signatureBox = this.createSignatureBox(options.signature);
   ...
   */
  private createSignatureBox(signature?: string): Buffer {
    if (signature) {
      // Use real signature from c2pa-service (base64 encoded)
      const signatureBuffer = Buffer.from(signature, 'base64');
      return this.createBox('sign', signatureBuffer);
    }

    // Fallback to placeholder if no signature provided (should not happen in production)
    const placeholderSignature = Buffer.from('CredLink-Signature-Placeholder', 'utf8');
    return this.createBox('sign', placeholderSignature);
  }
