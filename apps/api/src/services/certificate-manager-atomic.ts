...
export type CertificateRotationResult = RotationResult;
...
export class AtomicCertificateManager {
    ...
    async getSigningKey(): Promise<string> {
        return this.getSigningKeyAsync();
    }

    private parseCertificate(certificateData: string): Certificate {
        const fingerprint = createHash('sha256').update(certificateData).digest('hex');
        return {
            pem: certificateData,
            fingerprint,
            id: fingerprint,
    ...
    private async fetchNewCertificate(): Promise<Certificate> {
        ...
        newCertificate.id = createHash('sha256').update(newCertificate.pem + Date.now()).digest('hex');
    ...
    async rollbackCertificate(): Promise<RotationResult> {
        ...
        this.rotationStatus.previousCertificate = undefined;
    ...
    async performHealthCheck(): Promise<{
        ...
        try {
            if (this.destroyed) {
                issues.push('Certificate manager has been destroyed');
                healthy = false;
            } else if (!this.currentCertificate) {
                issues.push('No current certificate loaded');
                healthy = false;
            } else {
                // Check certificate expiry
                const now = Date.now();
                const expiryTime = this.currentCertificate.expiresAt.getTime();
                ...
                if (timeUntilExpiry < 0) {
                    ...
                    issues.push('Certificate expires within 24 hours');
                    healthy = false;
                }

                // Check rotation status
                if (this.rotationStatus.rotationInProgress) {
                    issues.push('Certificate rotation in progress');
                }
            }
        } catch (error) {
        ...
        return {
            healthy,
            certificate: this.currentCertificate || undefined,
            issues,
            timeUntilExpiry
        };
    }
