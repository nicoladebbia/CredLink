  ...
  private async extractExifData(buffer: Buffer): Promise<any> {
    try {
      ...
      if (metadata.exif) {
        ...
        // Return the parsed data (exif-parser provides tags and imageSize)
        return {
          tags: result.tags || {},
      ...
      // If EXIF parsing fails, return empty object (graceful degradation)
      console.warn('EXIF parsing failed:', (error as Error).message);
      return {};
    }
  }
}
