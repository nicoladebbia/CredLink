...
describe('Embedding Tests - Day 6-8', () => {
  ...
  describe('EXIF Data Preservation Tests', () => {
    ...
    it('should add CredLink EXIF fields', async () => {
      ...
      // Try parsing EXIF buffer, fall back to string search
      const exifParser = require('exif-parser');
      let foundCredLink = false;
      ...
      try {
        ...
        if (tags.ImageDescription && tags.ImageDescription.includes('CredLink:')) {
          foundCredLink = true;
        }
        if (tags.Software === 'CredLink/1.0') {
          foundSoftware = true;
        }
      } catch (e) {
        // Parser failed, try string search
        const exifString = metadata.exif!.toString('utf8');
        if (exifString.includes('CredLink:')) {
          foundCredLink = true;
        }
        if (exifString.includes('CredLink/1.0')) {
          foundSoftware = true;
        }
      }

      expect(foundCredLink).toBe(true);
      expect(foundSoftware).toBe(true);
    });
  ...
  describe('Size Optimization Tests', () => {
    it('should not increase file size excessively', async () => {
      ...
      // Should not increase by more than 30% (EXIF + JUMBF + re-encoding)
