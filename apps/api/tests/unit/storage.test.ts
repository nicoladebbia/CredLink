...
describe('Storage Unit Tests', () => {
  ...
  describe('StorageManager Integration', () => {
    ...
    beforeEach(() => {
      // Mock storage providers to avoid S3 dependency
      const mockProvider = {
        store: jest.fn().mockResolvedValue('proof-uri'),
        retrieve: jest.fn().mockResolvedValue(null),
        delete: jest.fn().mockResolvedValue(undefined),
        exists: jest.fn().mockResolvedValue(false),
        list: jest.fn().mockResolvedValue([]),
        healthCheck: jest.fn().mockResolvedValue({ healthy: true, provider: 'mock', latency: 0 }),
        getStats: jest.fn().mockResolvedValue({ totalObjects: 0, totalSize: 0, provider: 'mock' })
      };
      ...
      storageManager = new StorageManager({
        ...
        fallbackProvider: mockProvider as any
      });
    });
