...
describe('AdvancedExtractor', () => {
  ...
  describe('Multi-Format Extraction', () => {
    ...
    it('should extract from WebP', async () => {
      ...
      const embedded = await embedder.embedProofInImage(webpBuffer, manifest, proofUri);
  ...
  describe('Performance Metrics', () => {
    ...
    it('should handle large images efficiently', async () => {
      ...
      expect(result.metadata.imageSize).toBeGreaterThan(10000); // After JPEG compression
    });
  });
  ...
  describe('Partial Recovery', () => {
    it('should recover proof URI from corrupted metadata', async () => {
      ...
      // Should attempt partial recovery if needed (or succeed with earlier method)
      expect(result.success || result.metadata.methodsAttempted.includes('partial-recovery')).toBe(true);
    });
  });
