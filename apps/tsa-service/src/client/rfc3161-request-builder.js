...
export class RFC3161RequestBuilder {
    static DEFAULT_HASH_ALGORITHM = '2.16.840.1.101.3.4.2.1'; // SHA-256
    // Security constants
    static MIN_NONCE_BYTES = 8;
    static MAX_NONCE_BYTES = 32;
    static MIN_IMPRINT_SIZE = 32;
    static MAX_IMPRINT_SIZE = 512;
    // Secure hash algorithms (SHA-2 family only)
    static SECURE_ALGORITHMS = new Set([
    ...
    static buildRequest(params) {
        ...
        if (!Array.isArray(imprint) ||
            ...
            throw new Error(`Invalid imprint size: ${imprint.length} bytes. ` +
        ...
        if (!RFC3161RequestBuilder.SECURE_ALGORITHMS.has(selectedHashAlg)) {
            throw new Error(`Insecure or unsupported hash algorithm: ${selectedHashAlg}. ` +
    ...
    static generateNonce(bytes = 16) {
        ...
        if (typeof bytes !== 'number' ||
            ...
            throw new Error(`Invalid nonce size: ${bytes} bytes. ` +
     ...
     */
    static validateInputs(imprint, hashAlg, reqPolicy, nonce, certReq) {
        // Validate imprint
    ...
    static validateRequestParams(params) {
        ...
        if (params.imprint.length < RFC3161RequestBuilder.MIN_IMPRINT_SIZE ||
            ...
            throw new Error(`Message imprint size out of bounds. ` +
                `Must be between ${RFC3161RequestBuilder.MIN_IMPRINT_SIZE} and ${RFC3161RequestBuilder.MAX_IMPRINT_SIZE} bytes`);
        }
        if (params.nonce) {
            if (params.nonce < 0) {
...
//# sourceMappingURL=rfc3161-request-builder.js.map