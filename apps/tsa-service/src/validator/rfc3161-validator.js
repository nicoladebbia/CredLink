...
export class RFC3161Validator {
    static TIMESTAMPING_EKU = '1.3.6.1.5.5.7.3.8'; // id-kp-timeStamping
    static CRITICAL_EKU = true; // RFC 3161 §2.3 - MUST be critical
    // Security constants
    static MAX_FUTURE_TIME_MS = 60 * 60 * 1000; // 1 hour
    static MIN_PAST_YEAR = 2000;
    static MAX_SERIAL_NUMBER = BigInt('18446744073709551615'); // 2^64 - 1
    static MAX_NONCE_VALUE = BigInt('115792089237316195423570985008687907853269984665640564039457584007913129639935'); // 2^256 - 1
    /**
     ...
     */
    async validateToken(token, expectedImprint, tenantPolicy, expectedNonce) {
     ...
     */
    validateTSTInfo(tstInfo, expectedImprint, expectedNonce) {
        const result = { valid: false };
        // Input validation
        ...
        if (tstInfo.extensions) {
            ...
            const unknownExts = Object.keys(tstInfo.extensions).filter(oid => !this.isKnownExtension(oid));
     ...
     */
    async validateCertificateChain(token, tenantPolicy) {
        const result = { valid: false };
        // Input validation
        ...
        const timestampingEKU = signerCert.extensions.find(ext => ext && typeof ext.oid === 'string' && ext.oid === RFC3161Validator.TIMESTAMPING_EKU);
        if (!timestampingEKU) {
        ...
        const chainValid = await this.validateChainToAnchor(token.certChain || [], signerCert, trustedAnchor);
        if (!chainValid) {
    ...
    messageImprintsMatch(imprint1, imprint2) {
        // Input validation
    ...
    findTrustedAnchor(signerCert, anchors) {
    ...
    async validateChainToAnchor(chain, signerCert, anchor) {
        // CRITICAL SECURITY WARNING: Placeholder implementation
   ...
   *
     ...
     */
    validateESSCertIDv2(certHash, certificate) {
        // Input validation
     ...
     */
    validateInputs(token, expectedImprint, tenantPolicy, expectedNonce) {
...
//# sourceMappingURL=rfc3161-validator.js.map