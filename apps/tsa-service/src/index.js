...
app.get('/metrics', async (c) => {
    ...
    const metrics = [
        ...
        ...Object.entries(status.providers).map(([id, health]) => `tsa_provider_health{provider="${id.replace(/"/g, '\"')}"} ${health.healthy ? 1 : 0}`),
        '',
        `# HELP tsa_provider_latency Provider latency in milliseconds`,
        `# TYPE tsa_provider_latency gauge`,
        ...Object.entries(status.providers).map(([id, health]) => `tsa_provider_latency{provider="${id.replace(/"/g, '\"')}"} ${health.latencyMs}`),
        '',
