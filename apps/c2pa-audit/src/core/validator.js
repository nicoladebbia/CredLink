...
export class ManifestValidator {
    ...
    static MAX_MANIFEST_SIZE = 10 * 1024 * 1024; // 10MB
    static MAX_ASSERTIONS = 1000;
    static MAX_INGREDIENTS = 100;
    static MAX_NESTING_DEPTH = 10;
    // Allowed signature algorithms
    static ALLOWED_SIGNATURE_ALGORITHMS = new Set([
        'ES256',
        'ES384',
        'ES512',
        'RS256',
    ...
    static REQUIRED_ASSERTIONS = new Set([
        'c2pa.actions',
        'c2pa.hash_data'
    ]);
    /**
     ...
     */
    static async validateManifest(manifest, trustAnchors) {
        // Input validation
        ...
        try {
            ...
            const signatureValidation = this.validateClaimSignature(manifest.claim_signature, trustAnchors);
     ...
     */
    static validateManifestStructure(manifest) {
        const codes = [];
        let isValid = true;
     ...
     */
    static validateClaimSignature(claimSignature, certificates) {
        const codes = [];
        let isValid = true;
        try {
     ...
     */
    static validateAssertions(assertions) {
        const codes = [];
        let isValid = true;
     ...
     * @returns Assertion validation result
     */
    static validateSingleAssertion(assertion) {
        const codes = [];
        let isValid = true;
     ...
     */
    static async validateTimestampToken(manifest) {
        const codes = [];
        let isValid = true;
        try {
     ...
     */
    static getMaxDepth(ingredient, currentDepth = 1) {
     ...
     */
    static generateValidationSummary(isValid, codes) {
        if (isValid) {
            return 'Manifest validation passed successfully';
        }
        const errorCount = codes.filter(code => code.includes('invalid') || code.includes('missing') || code.includes('untrusted')).length;
