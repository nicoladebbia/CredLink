...
export function buildLicenseAssertion(metadata) {
    ...
    return {
        ...
        iptc: {
            LicensorName: license.licensor_name || getProviderName(metadata.provider),
            WebStatement: license.license_url,
            UsageTerms: license.usage_terms || 'See provider terms',
            Copyright: `© ${license.licensor_name || getProviderName(metadata.provider)}`,
...
export function buildReleaseAssertion(metadata) {
    return {
        ...
        release_notes: metadata.releases.release_notes
    };
}
/**
...
export function buildProviderManifestAssertion(metadata, tenantId, issuerDomain) {
    ...
    return {
        version: '1.0',
        asset: {
            primary_url: metadata.asset_url || metadata.source_urls.asset_page || '',
        ...
        links: {
            source_provider: metadata.source_urls.api_endpoint,
            license_terms: metadata.license.license_url,
            asset_page: metadata.source_urls.asset_page
        },
        signature: {
            alg: 'ES256',
...
export function generateManifestHash(assertion) {
    const manifestJson = JSON.stringify(assertion, null, 2);
...
export function getManifestStoragePath(manifestHash) {
    return `manifests/${manifestHash}.c2pa.json`;
}
/**
