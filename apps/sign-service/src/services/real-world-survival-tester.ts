...
import {
  ...
  SeverityStats,
  FailureAnalysis
} from '../tests/survival/survival-types';
...
export class RealWorldSurvivalTester {
    ...
    if (failedScenarios.length > 0) {
      ...
      failedScenarios.forEach(scenario => {
        Object.entries(scenario.failureReasons).forEach(([reason, count]) => {
          commonFailures.set(reason, (commonFailures.get(reason) || 0) + (count as number));
        });
      });
  ...
  analyzeFailures(results: RealWorldTestResult[]): FailureAnalysis {
    ...
    const totalFailures = failedResults.reduce((sum, r) => 
      sum + Object.values(r.failureReasons).reduce((s, c) => (s as number) + (c as number), 0), 0
    );
    ...
    failedResults.forEach(result => {
      Object.entries(result.failureReasons).forEach(([reason, count]) => {
        byReason[reason] = (byReason[reason] || 0) + (count as number);
      });
