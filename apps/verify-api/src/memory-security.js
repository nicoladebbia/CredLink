...
class MemorySecurityManager {
     ...
     */
    static validateBufferSize(buffer, source) {
        if (buffer.length > this.LIMITS.maxBufferSize) {
            throw new Error(`BUFFER_OVERFLOW_DETECTED: ${source} buffer size ${buffer.length} exceeds limit ${this.LIMITS.maxBufferSize}`);
        }
        if (buffer.length === 0) {
     ...
     */
    static createSafeBuffer(size, source) {
        if (size > this.LIMITS.maxBufferSize) {
            throw new Error(`BUFFER_CREATION_BLOCKED: ${source} requested size ${size} exceeds limit ${this.LIMITS.maxBufferSize}`);
        }
        if (size < 0) {
     ...
     */
    static validateArraySize(array, source) {
        if (array.length > this.LIMITS.maxArraySize) {
            throw new Error(`ARRAY_OVERFLOW_DETECTED: ${source} array size ${array.length} exceeds limit ${this.LIMITS.maxArraySize}`);
        }
    }
    /**
     * CRITICAL: String length validation to prevent memory exhaustion
     */
    static validateStringLength(str, source) {
        if (str.length > this.LIMITS.maxStringSize) {
            throw new Error(`STRING_OVERFLOW_DETECTED: ${source} string length ${str.length} exceeds limit ${this.LIMITS.maxStringSize}`);
        }
    }
    /**
     * CRITICAL: Safe JSON parsing with memory limits
     */
    static safeJsonParse(json, source) {
     ...
     */
    static validateObjectDepth(obj, source, currentDepth, maxDepth) {
        if (currentDepth > maxDepth) {
            throw new Error(`OBJECT_DEPTH_EXCEEDED: ${source} depth ${currentDepth} exceeds limit ${maxDepth}`);
        }
    ...
    static detectHeapSpraying() {
        ...
        if (heapUsedMB > 500) { // 500MB threshold
            ...
            if (this.activeBuffers.size > this.LIMITS.maxConcurrentBuffers) {
                throw new Error(`HEAP_SPRAYING_ATTACK: Too many active buffers ${this.activeBuffers.size}`);
            }
        }
     ...
     */
    static safeBufferCopy(dest, src, source) {
        this.validateBufferSize(dest, `${source}_dest`);
        this.validateBufferSize(src, `${source}_src`);
        if (src.length > dest.length) {
            throw new Error(`BUFFER_COPY_OVERFLOW: ${source} source ${src.length} > destination ${dest.length}`);
        }
        dest.set(src);
    }
    /**
     ...
     */
    static trackBuffer(buffer, source) {
...
//# sourceMappingURL=memory-security.js.map