...
interface FalsePositiveTestConfig {
  endpoint: string;
  route: string;
  requestRate: number;
  duration: number;
...
interface FalsePositiveTestResult {
  config: FalsePositiveTestConfig;
  startTime: string;
  endTime: string;
...
class FalsePositiveTester {
  private config: FalsePositiveTestConfig;
  private results: FalsePositiveTestResult;

  constructor(config: FalsePositiveTestConfig) {
    this.config = config;
    this.results = {
      config,
  ...
  async run(): Promise<FalsePositiveTestResult> {
  ...
  private async sendLegitimateRequest(): Promise<void> {
    try {
  ...
  private async checkFinalState(): Promise<void> {
    try {
...
async function main() {
  ...
  const config: FalsePositiveTestConfig = {
  ...
  // Save results
  const resultsFile = join(process.cwd(), 'false-positive-results.json');
