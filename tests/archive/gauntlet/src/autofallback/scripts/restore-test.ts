...
interface RestoreTestConfig {
  ...
  route: string;
  requestRate: number;
  hysteresisPeriod: number; // seconds
...
class RestoreTest {
  private config: RestoreTestConfig;
  private results: RestoreTestResult;
  ...
  async runTest(): Promise<RestoreTestResult> {
  ...
  private async generateCleanTraffic(endTime: number): Promise<void> {
  ...
  private async sendCleanRequest(requestId: number): Promise<void> {
  ...
  private async startPolicyMonitoring(endTime: number): Promise<void> {
  ...
  private saveResults(): void {
    const resultsPath = join(process.cwd(), 'artifacts', 'autofallback', 'restore-results.json');
...
Examples:
  ...
  const config: RestoreTestConfig = {
    endpoint: '',
    route: '',
    requestRate: 5,
