...
class BreakGlassManager {
  ...
  private results: BreakGlassResult;
  ...
  async execute(): Promise<BreakGlassResult> {
  ...
  private async setBreakGlass(): Promise<BreakGlassResult> {
    try {
      const payload = {
  ...
  private async clearBreakGlass(): Promise<BreakGlassResult> {
    try {
      const response = await fetch(`${this.config.endpoint}/_c2/breakglass?route=${encodeURIComponent(this.config.route)}`, {
        method: 'DELETE',
        headers: {
...
async function main() {
  ...
  const action = args[0] as 'set' | 'clear';
  ...
  // Save results
  const resultsFile = join(process.cwd(), 'break-glass-results.json');
