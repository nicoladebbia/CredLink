...
export class C2Badge extends HTMLElement {
  ...
  private badgeButton: HTMLButtonElement | null = null;
  ...
  private render(): void {
    ...
    const badgeClone = BADGE_TEMPLATE.content.cloneNode(true) as DocumentFragment;
  ...
  private openModal(): void {
    ...
    const modalClone = MODAL_TEMPLATE.content.cloneNode(true) as DocumentFragment;
    this.modal = modalClone.querySelector('.c2-modal-overlay') as HTMLElement;
  ...
  private bindModalEvents(): void {
    ...
    // Focus management
    const closeButtonElement = closeButton as HTMLButtonElement;
  ...
  private createAssertionElement(text: string, isAIGenerated: boolean): HTMLElement {
  ...
  private renderResult(): void {
    ...
    const signerKey = body.querySelector('#signer-key') as HTMLElement;
    const signerOrg = body.querySelector('#signer-org') as HTMLElement;
    const signerTrusted = body.querySelector('#signer-trusted') as HTMLElement;
    
    if (signerName) signerName.textContent = this.currentResult.signer.name;
    if (signerKey) signerKey.textContent = this.currentResult.signer.key_id;
    if (signerOrg) signerOrg.textContent = this.currentResult.signer.organization || 'N/A';
    if (signerTrusted) signerTrusted.textContent = this.currentResult.signer.trusted ? 'Yes' : 'No';

    // Update assertions
    const assertionsContainer = body.querySelector('#assertions') as HTMLElement;
    ...
    const decisionPath = body.querySelector('#decision-path') as HTMLElement;
    if (decisionPath) {
  ...
  private renderError(message: string): void {
    ...
    const errorClone = ERROR_TEMPLATE.content.cloneNode(true) as DocumentFragment;
    body.appendChild(errorClone);

    const errorMessageElement = body.querySelector('#error-message') as HTMLElement;
    ...
    // Bind retry button
    const retryButton = body.querySelector('#retry-verification') as HTMLButtonElement;
  ...
  private bindActionButtons(): void {
    ...
    const viewManifestButton = this.modal.querySelector('#view-manifest') as HTMLButtonElement;
    const copyUrlButton = this.modal.querySelector('#copy-url') as HTMLButtonElement;
