...
export class ManifestStore {
     ...
     */
    async storeManifest(objectKey, content, metadata) {
        try {
            // Rate limiting check
     ...
     */
    async checkIntegrity(objectKey, content) {
        try {
            // Validate object key format
            if (!this.VALID_OBJECT_KEY_PATTERN.test(objectKey)) {
                return {
    ...
    async listObjects(options = {}) {
        try {
            ...
            const limit = Math.min(Math.max(options.limit || this.DEFAULT_LIST_LIMIT, 1), this.MAX_LIST_LIMIT);
            let startIndex = 0;
    ...
    async getMetrics() {
        try {
            ...
            const readFailures24h = recentAudits.filter(audit => audit.operation === 'access' && audit.result === 'failure').length;
    ...
    async getAuditRecords(options = {}) {
        try {
            ...
            // Apply pagination with strict limits
            const limit = Math.min(Math.max(options.limit || this.DEFAULT_AUDIT_LIMIT, 1), this.MAX_AUDIT_LIMIT);
    ...
    validateSignedUrlRequest(request) {
    ...
    validateManifestContent(content, metadata) {
    ...
    sanitizeError(error) {
        ...
        if (error instanceof Error)
            return error.message.substring(0, 200).replace(/[<>"'&]/g, '');
        return 'Unknown error';
    }
