...
describe('ManifestStore', () => {
    ...
    test('should reject invalid object key format', async () => {
        ...
        await assert.rejects(() => store.generateSignedUrl(request), /Invalid object key: must be SHA-256 hash with \.c2pa extension/);
    });
    test('should reject disallowed content type', async () => {
        ...
        await assert.rejects(() => store.generateSignedUrl(request), /Content type text\/plain not allowed/);
    });
    ...
    test('should enforce write-once semantics', async () => {
        ...
        await assert.rejects(() => store.storeManifest(objectKey, content, metadata), /Object already exists: write-once semantics prohibit overwrites/);
    });
...
//# sourceMappingURL=manifest-store.test.js.map