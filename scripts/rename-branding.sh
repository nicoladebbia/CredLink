#!/bin/bash
# Rename credlink → CredLink across entire codebase

set -e

echo "🏷️  Rebranding: credlink → CredLink"
echo "========================================"
echo ""

# Find and replace in files
echo "📝 Updating package.json files..."
find . -name "package.json" -type f -exec sed -i '' 's/"credlink"/"credlink"/g' {} \;
...
find . -name "*.go" -type f -exec sed -i '' 's/credlink/credlink/g' {} \;

echo "📝 Updating HTML files..."
find . -name "*.html" -type f -exec sed -i '' 's/credlink/credlink/g' {} \;
find . -name "*.html" -type f -exec sed -i '' 's/C2 Concierge/CredLink/g' {} \;

echo "📝 Updating YAML files..."
find . -name "*.yaml" -o -name "*.yml" -type f -exec sed -i '' 's/credlink/credlink/g' {} \;
...
if [ -f "cli/go.mod" ]; then
    ...
    sed -i '' 's|github.com/credlink/cli|github.com/credlink/cli|g' cli/go.mod
fi
...
echo "  1. Update GitHub repo name: credlink → CredLink"
