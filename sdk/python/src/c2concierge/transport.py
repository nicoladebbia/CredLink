"""
Transport layer for the C2 Concierge SDK
    ...
    def _initialize_opentelemetry(self):
        ...
        try:
            ...
            self.tracer = trace.get_tracer(
                self.otel_config.get("service_name", "c2concierge-sdk"),
                self.otel_config.get("service_version", "1.3.0")
            )
            
            print("C2C SDK: OpenTelemetry enabled")
        except ImportError:
            print("C2C SDK: OpenTelemetry packages not installed, telemetry disabled")
            self.enabled = False
        except Exception as e:
            print(f"C2C SDK: Failed to initialize OpenTelemetry: {e}")
            self.enabled = False
...
class HttpTransport:
    ...
    DEFAULT_CONFIG = {
        ...
        "user_agent": "c2c-sdk/python/1.3.0",
    }
